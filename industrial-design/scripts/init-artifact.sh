...
# Install shadcn/ui components — use tarball if available, otherwise npx
if [ -f "$COMPONENTS_TARBALL" ]; then
  echo "📦 Extracting shadcn/ui components from tarball..."
  tar -xzf "$COMPONENTS_TARBALL" -C src/
else
  echo "📦 Installing shadcn/ui components via npx (tarball not found)..."
  npx shadcn-ui@latest add --all --yes
fi
